...
static WCHAR g_midimap_keyW[256] = L"Software\\Microsoft\\Windows\\CurrentVersion\\Multimedia\\MIDIMap";
...
{
    { IDS_AUDIO_SPEAKER_7POINT1_WIDE, KSAUDIO_SPEAKER_7POINT1 },
...
static BOOL get_driver_name(IMMDeviceEnumerator *devenum, PROPVARIANT *pv)
{
    ...
    if(FAILED(hr)) {
        ERR("Could not get Wine info device.\n");
        return FALSE;
    }

    hr = IMMDevice_OpenPropertyStore(device, STGM_READ, &ps);
    if(FAILED(hr)){
        ERR("Could not get property store for device (0x%p).\n", device);
...
static void set_reg_midi_device(HWND hDlg, int dlgitem)
{
    ...
    if(!id || id == (void*)CB_ERR)
        set_reg_key(HKEY_CURRENT_USER, g_midimap_keyW, L"szPname", NULL);
    else
        set_reg_key(HKEY_CURRENT_USER, g_midimap_keyW, L"szPname", id);
}
