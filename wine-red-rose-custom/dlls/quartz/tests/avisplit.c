...
static void test_seeking(void)
{
    ...
    for (i = 0; i < ARRAY_SIZE(rounding_tests); ++i)
    {
        LONGLONG result = 0xdeadbeef;

        hr = IMediaSeeking_ConvertTimeFormat(seeking, &result, &TIME_FORMAT_FRAME,
                rounding_tests[i].media_time, &TIME_FORMAT_MEDIA_TIME);

        todo_wine ok(hr == S_OK, "test %u hr %#lx.\n", i, hr);

        if (hr == S_OK)
        {
            ok(result == rounding_tests[i].frame,
                   "wrong frame %s at test %u.\n", wine_dbgstr_longlong(result), i);
        }
        else
        {
            ok(result == 0xdeadbeef, "result was modified at test %u: %s.\n",
