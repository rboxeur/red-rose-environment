...
static void test_video_window_style(IVideoWindow *window, HWND hwnd, HWND our_hwnd)
{
    ...
    hr = IVideoWindow_put_WindowStyle(window,
            (style | WS_VISIBLE | WS_POPUP | WS_GROUP | WS_TABSTOP) & ~WS_CLIPCHILDREN);
    ok(hr == S_OK, "Got hr %#lx.\n", hr);

    hr = IVideoWindow_get_WindowStyle(window, &style);
    ok(hr == S_OK, "Got hr %#lx.\n", hr);
    ok(style == (WS_CLIPSIBLINGS | WS_OVERLAPPEDWINDOW | WS_POPUP | WS_GROUP | WS_TABSTOP),
            "Got style %#lx.\n", style);
