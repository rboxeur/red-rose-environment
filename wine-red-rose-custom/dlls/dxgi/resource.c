...
static HRESULT STDMETHODCALLTYPE dxgi_resource_inner_QueryInterface(IUnknown *iface, REFIID riid, void **out)
{
    ...
    else if ((!resource->is_subresource && (IsEqualGUID(riid, &IID_IDXGIResource)
...
static HRESULT STDMETHODCALLTYPE dxgi_surface_GetDesc(IDXGISurface2 *iface, DXGI_SURFACE_DESC *desc)
{
    ...
    wined3d_mutex_lock();
    wined3d_resource_get_sub_resource_desc(resource->wined3d_resource, resource->subresource_idx, &wined3d_desc);
    wined3d_mutex_unlock();
    desc->Width = wined3d_desc.width;
    desc->Height = wined3d_desc.height;
    if (resource->is_subresource && resource->is_buffer)
        ...
        IDXGIResource1 *parent_resource, unsigned int subresource_index)
{
    ...
    resource->dc = NULL;
    resource->is_buffer = desc.resource_type == WINED3D_RTYPE_BUFFER;
