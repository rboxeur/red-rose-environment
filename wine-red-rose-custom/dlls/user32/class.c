 ...
 */
BOOL WINAPI UnregisterClassA( LPCSTR class_name, HINSTANCE instance )
{
    struct client_menu_name menu_name;
    WCHAR nameW[MAX_ATOM_LEN + 1];
    UNICODE_STRING name = RTL_CONSTANT_STRING(nameW), version;
    ...
    init_class_name_ansi( &name, class_name );
    get_class_version( &name, &version, FALSE );
 ...
 */
BOOL WINAPI GetClassInfoExA( HINSTANCE instance, const char *class_name, WNDCLASSEXA *wc )
{
    WCHAR nameW[MAX_ATOM_LEN + 1];
    UNICODE_STRING name = RTL_CONSTANT_STRING(nameW), version;
    ATOM atom;
    ...
    init_class_name_ansi( &name, class_name );
    get_class_version( &name, &version, TRUE );

    if (!instance) instance = user32_module;
    if (!(atom = NtUserGetClassInfoEx( instance, &name, (WNDCLASSEXW *)wc, NULL, TRUE )))
    {
        TRACE( "%s %p -> not found\n", debugstr_us(&name), instance );
 ...
 */
BOOL WINAPI GetClassInfoExW( HINSTANCE instance, const WCHAR *class_name, WNDCLASSEXW *wc )
{
    WCHAR nameW[MAX_ATOM_LEN + 1];
    UNICODE_STRING name = RTL_CONSTANT_STRING(nameW), version;
    ATOM atom;
    ...
    if (!instance) instance = user32_module;
    if (!(atom = NtUserGetClassInfoEx( instance, &name, wc, NULL, FALSE )))
    {
        TRACE( "%s %p -> not found\n", debugstr_us(&name), instance );
        SetLastError( ERROR_CLASS_DOES_NOT_EXIST );
        return 0;
