                                    ...
                                    TW_MEMREF pData)
{
    ...
    else
    {
        ...
        while (i>=-1)
          {
            /* Some backends can deliver more data than requestes, especially
             * in ADF-Mode.
             * Dummy-Read until EOF, else the rest of the image might be read
             * as another (nearly empty) frame. Observed with brscan4 driver */
            BYTE *line = (BYTE *) malloc(activeDS.frame_params.bytes_per_line);
            if (line)
              {
                int retlen;
                struct read_data_params params = { line, activeDS.frame_params.bytes_per_line, &retlen };
                if (SANE_CALL( read_data, &params ) != TWCC_SUCCESS ||
                    retlen != activeDS.frame_params.bytes_per_line)
                  {
                    // EOF reached
                    i = -2;
                  }
