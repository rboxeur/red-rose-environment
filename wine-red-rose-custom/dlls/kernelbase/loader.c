...
static WCHAR mui_locale[LOCALE_NAME_MAX_LENGTH];    /* MUI work variables   */
...

{
    ...
    if (!(GetModuleFileNameW(module, module_name, MAX_PATH)))   /* Acquire the base resource file name */
        return module;
...
HGLOBAL WINAPI DECLSPEC_HOTPATCH LoadResource( HINSTANCE module, HRSRC rsrc )
{
    ...
    if (((HMODULE)rsrc < module) || ((mui_module > module) && ((HMODULE)rsrc > mui_module))) 
        module = mui_module;
