    ...
    const WICRect *prc, UINT cbStride, UINT cbBufferSize, BYTE *pbBuffer)
{
    ...
    if ((This->swap_xy || This->flip_x) && This->bpp < 8)
    {
    ...
    {
        UINT bytes_per_pixel = This->bpp / 8, stride = srcwidth * bytes_per_pixel;
        BYTE *bits;

        bits = malloc(srcheight * stride);
        ...
        rc.Width = srcwidth;
        rc.Height = srcheight;
        hr = IWICBitmapSource_CopyPixels(This->source, &rc, stride, srcheight * stride, bits);
        if (FAILED(hr))
        {
            free(bits);
            return hr;
        ...
        for (y = prc->Y; y - prc->Y < prc->Height; y++)
        {
            ...
            {
                BYTE *src = bits + (width - x - 1) * stride + y * bytes_per_pixel;
                BYTE *dst = pbBuffer + y * cbStride + x * bytes_per_pixel;
    ...
    else if (This->swap_xy || This->flip_x)
    {
        FIXME("flipping x and rotating are not implemented for %u bpp bitmap\n", This->bpp);
        return E_NOTIMPL;
    }
