...
/* Formats supported in Windows as both source and destination, but not supported in Wine */
static const WICPixelFormatGUID *missing_native_formats[] = {
...
static BOOL is_missing_native_format(const WICPixelFormatGUID *format)
{
    UINT i;

    for (i=0; missing_native_formats[i]; i++)
        if (IsEqualGUID(missing_native_formats[i], format)) return TRUE;
    ...
    BOOL *pfCanConvert)
{
    ...
    if (!srcinfo)
    {
        if (is_missing_native_format(srcPixelFormat) || IsEqualGUID(&GUID_WICPixelFormat32bppR10G10B10A2, srcPixelFormat))
    ...
    if (!dstinfo)
    {
        if (is_missing_native_format(dstPixelFormat))
    ...
    if (!dstinfo->copy_function ||
        FAILED(dstinfo->copy_function(This, NULL, 0, 0, NULL, dstinfo->format)))
    {
