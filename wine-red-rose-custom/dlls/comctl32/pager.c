...
typedef struct
{
    ...
    COMCTL32_UnicodeBuffer unicodeBuffer;/* used in WN_NOTIFY forwarding if bUnicode */
} PAGER_INFO;
...
PAGER_Destroy (PAGER_INFO *infoPtr)
{
    ...
    Free (infoPtr->unicodeBuffer.address);
    Free (infoPtr);
    return 0;
}
...
{
    return infoPtr->bUnicode ? SendMessageW(infoPtr->hwndNotify, WM_NOTIFY, hdr->idFrom, (LPARAM)hdr) : COMCTL32_ForwardNotifyToAnsiWindow(infoPtr->hwndNotify, hdr, &infoPtr->unicodeBuffer);
}
