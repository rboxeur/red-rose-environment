...
static void hook_nav_complete(HHInfo *info, BOOL init)
{
    ...
    if (init)
    {
        ...
        IConnectionPoint_Advise(point, (IUnknown *)info->navigate_sink, &dw);
    }
    else
    {
        IConnectionPoint_Unadvise(point, dw);
        IDispatch_Release((IDispatch *)info->navigate_sink);
    }
...
static BOOL HH_AddHTMLPane(HHInfo *pHHInfo)
{
    ...
    hook_nav_complete(pHHInfo, TRUE);
