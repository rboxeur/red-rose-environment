...
/* Static array for storing predefined power schemes */
static const power_scheme_info valid_power_schemes[] = 
{
    {&GUID_TYPICAL_POWER_SAVINGS, L"Balanced"},
    {&GUID_MAX_POWER_SAVINGS, L"Power Saver"},
    {&GUID_MIN_POWER_SAVINGS, L"High Performance"}
};
/* Static variable for storing the active power scheme, set to Balanced by default */
static GUID const *active_power_scheme = &GUID_TYPICAL_POWER_SAVINGS;
...
DWORD WINAPI PowerGetActiveScheme(HKEY UserRootPowerKey, GUID **polguid)
{
    ...
    TRACE("*polguid %p, active_power_scheme %s\n",*polguid,
          debugstr_guid(active_power_scheme));
    /* Windows SDK docs say use LocalFree to free this */
    *polguid = LocalAlloc(LMEM_FIXED, sizeof(GUID));
    if (*polguid == NULL)
    {
...
DWORD WINAPI PowerSetActiveScheme(HKEY UserRootPowerKey, GUID *polguid)
{
    ...
    for (i = 0; i < ARRAYSIZE(valid_power_schemes); i++)
    {
        if (IsEqualGUID(polguid, valid_power_schemes[i].guid))
        {
            active_power_scheme = polguid;
            return ERROR_SUCCESS;
        }
    }
	...
	DWORD *BufferSize)
{
    ...
    FIXME("should use registry\n");
    /* Check for invalid sub-group or power-setting GUIDs */
    if (SubGroup != NULL || PowerSettings != NULL)
    {
    ...
    for (i = 0; ; i++)
    {
        result = PowerEnumerate(NULL, NULL, NULL, ACCESS_SCHEME, i, (UCHAR *)&current_guid, &buffer_size);
        ...
        if (IsEqualGUID(Scheme, &current_guid))
        {
            name = valid_power_schemes[i].name;
            buffer_size = (wcslen(name) + 1) * sizeof(WCHAR);
            if (*BufferSize < buffer_size)
            {
                *BufferSize = buffer_size;
                return ERROR_MORE_DATA;
            }
            memcpy(Buffer, name, buffer_size);
            *BufferSize = buffer_size;
            return ERROR_SUCCESS;
        }
    }
    
    return ERROR_INVALID_PARAMETER; /* Scheme not found */
}
...
{
    /* Only care about ACCESS_SCHEME */
    if (flags != ACCESS_SCHEME || scheme != NULL || subgroup != NULL)
    {
        return ERROR_CALL_NOT_IMPLEMENTED;
    }
    if(index >= ARRAYSIZE(valid_power_schemes))
    {
        return ERROR_NO_MORE_ITEMS;
    }
    if(buffer == NULL || *buffer_size < sizeof(GUID))
    {
        *buffer_size = sizeof(GUID);
        return ERROR_MORE_DATA;
    }
    memcpy(buffer, valid_power_schemes[index].guid, sizeof(GUID));
    return ERROR_SUCCESS;
}
