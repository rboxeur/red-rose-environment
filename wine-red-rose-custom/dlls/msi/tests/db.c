...
static void test_table_mismatched_strref(void)
{
    ...
    /* Open as MSI database and verify rows read correctly despite mismatched strref */
    r = MsiOpenDatabaseW(msifileW, MSIDBOPEN_READONLY, &hdb);
    ok(r == ERROR_SUCCESS, "Expected ERROR_SUCCESS, got %d\n", r);
    ...
    ok(r == ERROR_SUCCESS, "Expected ERROR_SUCCESS, got %d\n", r);
    sz = sizeof(str);
    MsiRecordGetStringA(hrec, 2, str, &sz);
    ok(!strcmp(str, "one") || broken(!*str), "Expected \"one\", got \"%s\"\n", str);
    MsiCloseHandle(hrec);
    ...
    sz = sizeof(str);
    MsiRecordGetStringA(hrec, 2, str, &sz);
    ok(!strcmp(str, "two") || broken(!*str), "Expected \"two\", got \"%s\"\n", str);
    MsiCloseHandle(hrec);
    ...
    sz = sizeof(str);
    MsiRecordGetStringA(hrec, 2, str, &sz);
    ok(!strcmp(str, "three") || broken(!*str), "Expected \"three\", got \"%s\"\n", str);
    MsiCloseHandle(hrec);
    ...
    sz = sizeof(str);
    MsiRecordGetStringA(hrec, 2, str, &sz);
    ok(!strcmp(str, "four") || broken(!*str), "Expected \"four\", got \"%s\"\n", str);
    MsiCloseHandle(hrec);

    r = MsiViewFetch(hview, &hrec);
    ok(r == ERROR_SUCCESS || broken(r == ERROR_NO_MORE_ITEMS),
       "Expected ERROR_SUCCESS, got %d\n", r);
    if (r == ERROR_SUCCESS)
    {
        check_record(hrec, 2, "5", "five");
