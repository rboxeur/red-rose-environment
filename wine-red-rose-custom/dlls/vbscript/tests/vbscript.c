...
static void test_vbdisp_typeinfo(void)
{
    ...
    /* GetTypeAttr: class C has 1 public func (method) and 1 public var (x) */
    hr = ITypeInfo_GetTypeAttr(typeinfo, &attr);
    ok(hr == S_OK, "GetTypeAttr failed: %08lx\n", hr);
    ok(attr->typekind == TKIND_DISPATCH, "Unexpected typekind %u\n", attr->typekind);
    ok(attr->cFuncs == 1, "Unexpected cFuncs %u\n", attr->cFuncs);
    ok(attr->cVars == 1, "Unexpected cVars %u\n", attr->cVars);
    ok(attr->cImplTypes == 1, "Unexpected cImplTypes %u\n", attr->cImplTypes);
    ok(attr->wTypeFlags == TYPEFLAG_FDISPATCHABLE, "Unexpected wTypeFlags 0x%x\n", attr->wTypeFlags);
    ITypeInfo_ReleaseTypeAttr(typeinfo, attr);

    /* GetFuncDesc for the public sub 'method' (index 0) */
    hr = ITypeInfo_GetFuncDesc(typeinfo, 0, &funcdesc);
    ok(hr == S_OK, "GetFuncDesc(0) failed: %08lx\n", hr);
    ok(funcdesc->funckind == FUNC_DISPATCH, "Unexpected funckind %u\n", funcdesc->funckind);
    ok(funcdesc->invkind == INVOKE_FUNC, "Unexpected invkind %u\n", funcdesc->invkind);
    ok(funcdesc->cParams == 0, "Unexpected cParams %d\n", funcdesc->cParams);
    ok(funcdesc->elemdescFunc.tdesc.vt == VT_VOID, "Unexpected ret vt %d\n", funcdesc->elemdescFunc.tdesc.vt);
    hr = ITypeInfo_GetDocumentation(typeinfo, funcdesc->memid, &bstr, NULL, NULL, NULL);
    ok(hr == S_OK, "GetDocumentation failed: %08lx\n", hr);
    ok(!lstrcmpW(bstr, L"method"), "Unexpected func name %s\n", wine_dbgstr_w(bstr));
    SysFreeString(bstr);
    ITypeInfo_ReleaseFuncDesc(typeinfo, funcdesc);

    /* Out of range func */
    hr = ITypeInfo_GetFuncDesc(typeinfo, 1, &funcdesc);
    ok(hr == TYPE_E_ELEMENTNOTFOUND, "GetFuncDesc(1) returned: %08lx\n", hr);
