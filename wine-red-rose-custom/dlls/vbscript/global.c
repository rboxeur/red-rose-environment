...
static HRESULT Global_InStrB(BuiltinDisp *This, VARIANT *args, unsigned args_cnt, VARIANT *res)
{
    ...
    if(args_cnt == 2) {
        ...
        if(V_VT(args+1) == VT_BSTR) {
            ...
            hres = to_string(args+1, &conv_str2);
            if(FAILED(hres)) {
        ...
        if(V_VT(args+1) == VT_BSTR) {
            ...
            hres = to_string(args+1, &conv_str1);
            if(FAILED(hres))
                return hres;
            str1 = conv_str1;
        }
        if(V_VT(args+2) == VT_BSTR) {
            ...
            hres = to_string(args+2, &conv_str2);
            if(FAILED(hres)) {
                SysFreeString(conv_str1);
                return hres;
            }
...
static HRESULT Global_SetLocale(BuiltinDisp *This, VARIANT *args, unsigned args_cnt, VARIANT *res)
{
    ...
    if(!args_cnt) {
        This->ctx->lcid = GetUserDefaultLCID();
    ...
    case VT_BSTR: {
        int i;
        /* Try numeric conversion first (e.g. "1033") */
        hres = to_int(args, &i);
        if(SUCCEEDED(hres)) {
            new_lcid = i;
        }else {
            /* Try as locale name (e.g. "en-us") */
...
static HRESULT Global_DateValue(BuiltinDisp *This, VARIANT *arg, unsigned args_cnt, VARIANT *res)
{
    ...
    if(V_VT(arg) == VT_DATE) {
        ...
        V_VT(&v) = VT_EMPTY;
        /* FIXME: Use VariantChangeTypeEx with script lcid for locale-aware parsing (applies to CDate and other converters too). */
        hres = VariantChangeType(&v, arg, 0, VT_DATE);
        if(FAILED(hres))
            return MAKE_VBSERROR(VBSE_TYPE_MISMATCH);
        date = V_DATE(&v);
    ...
    /* Truncate toward zero to remove time fraction */
    date = date < 0 ? ceil(date) : floor(date);
    return return_date(res, date);
}
...
static HRESULT Global_TimeValue(BuiltinDisp *This, VARIANT *arg, unsigned args_cnt, VARIANT *res)
{
    ...
    /* Extract time fraction, removing the date part */
    date = date - (date < 0 ? ceil(date) : floor(date));
    return return_date(res, date);
}
...
static HRESULT Global_DateDiff(BuiltinDisp *This, VARIANT *args, unsigned args_cnt, VARIANT *res)
{
    ...
    else if(!wcsicmp(interval, L"ww"))
    {
        ...
        /* Use floor division so negative day numbers are handled correctly */
        result = (LONG)floor((double)(day2 - anchor) / 7.0)
               - (LONG)floor((double)(day1 - anchor) / 7.0);
    }
    else if(!wcsicmp(interval, L"h"))
    {
        result = (LONG)floor(date2 * 24.0) - (LONG)floor(date1 * 24.0);
    }
    else if(!wcsicmp(interval, L"n"))
    {
        result = (LONG)floor(date2 * 1440.0) - (LONG)floor(date1 * 1440.0);
    }
    else if(!wcsicmp(interval, L"s"))
    {
        result = (LONG)(floor(date2 * 86400.0 + 0.5) - floor(date1 * 86400.0 + 0.5));
    }
    else
    {
        WARN("Unrecognized interval %s.\n", debugstr_w(interval));
...
static HRESULT Global_Eval(BuiltinDisp *This, VARIANT *arg, unsigned args_cnt, VARIANT *res)
{
    ...
    hres = compile_script(This->ctx, V_BSTR(arg), NULL, NULL, 0, 0,
                          SCRIPTTEXT_ISEXPRESSION | SCRIPTTEXT_NOERRORREPORT, &code);
    if(FAILED(hres)) {
        /* Compilation failed. If compile_error didn't set up a proper VBS error,
         * fall back to VBSE_SYNTAX_ERROR. */
        if(HRESULT_FACILITY(This->ctx->ei.scode) != FACILITY_VBS) {
            clear_ei(&This->ctx->ei);
            This->ctx->ei.scode = MAKE_VBSERROR(VBSE_SYNTAX_ERROR);
            This->ctx->ei.bstrSource = get_vbscript_string(VBS_COMPILE_ERROR);
            This->ctx->ei.bstrDescription = get_vbscript_string(VBSE_SYNTAX_ERROR);
        }
    ...
    if(is_exec_local_scope(This->ctx->current_exec)) {
        ...
        return exec_script(This->ctx, TRUE, &code->main_code, NULL, NULL, res);
    }

    return exec_global_code(This->ctx, code, res);
}
...
static HRESULT Global_Execute(BuiltinDisp *This, VARIANT *arg, unsigned args_cnt, VARIANT *res)
{
    ...
    hres = compile_script(This->ctx, V_BSTR(arg), NULL, NULL, 0, 0, SCRIPTTEXT_NOERRORREPORT, &code);
    if(FAILED(hres)) {
        /* Compilation failed. If compile_error didn't set up a proper VBS error,
         * fall back to VBSE_SYNTAX_ERROR. */
        if(HRESULT_FACILITY(This->ctx->ei.scode) != FACILITY_VBS) {
