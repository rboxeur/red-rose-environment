...
static void test_yuv_to_rgb_blt(void)
{
    ...
    DWORD flags, chroma_pitch, chroma_height;
    DDSURFACEDESC2 surface_desc, lock_desc;
    ...
    test_textures[] =
    {
        {"UYVY", MAKEFOURCC('U', 'Y', 'V', 'Y'), 2, 1, 0, 0, 1, 0, 2, 2, 4, 4},
        {"YUY2", MAKEFOURCC('Y', 'U', 'Y', '2'), 2, 1, 0, 0, 0, 1, 3, 2, 4, 4},
        {"NV12", MAKEFOURCC('N', 'V', '1', '2'), 2, 2, 1, 1, 0, 0, 1, 1, 2, 2},
        {"YV12", MAKEFOURCC('Y', 'V', '1', '2'), 2, 2, 2, 1, 0, 0, 0, 1, 1, 1},
    };
    ...
    static const struct
    {
        ...
        BYTE v;
        UINT32 expected;
        /* some systems use SD values for HD. */
        BOOL test_for_broken;
        UINT32 broken_value;
    }
    test_colors[] =
    {
        {"black SD", 576, 0x10, 0x80, 0x80, 0x000000},
        {"white SD", 576, 0xeb, 0x80, 0x80, 0xffffff},
        {"red SD", 576, 0x51, 0x5a, 0xf0, 0xfe0000},
        {"green SD", 576, 0x91, 0x36, 0x22, 0x00ff01},
        {"blue SD", 576, 0x29, 0xf0, 0x6e, 0x0000ff},
        {"gray SD", 576, 0x7e, 0x80, 0x80, 0x808080},
        {"past black SD", 576, 0x00, 0x80, 0x80, 0x000000},
        {"past white SD", 576, 0xff, 0x80, 0x80, 0xffffff},
        {"zeros SD", 576, 0x00, 0x00, 0x00, 0x008700},
        /* Windows will treat anything with a height greater than 576 as HD and,
         * as per ITU-R Recommendation BT.709, change the color scheme */
        {"black HD", 580, 0x10, 0x80, 0x80, 0x000000},
        {"white HD", 580, 0xeb, 0x80, 0x80, 0xffffff},
        {"red HD", 580, 0x3f, 0x66, 0xf0, 0xff0000, TRUE, 0xe90002},
        {"green HD", 580, 0xad, 0x2a, 0x1a, 0x00ff01, TRUE, 0x14ff09},
        {"blue HD", 580, 0x20, 0xf0, 0x76, 0x0100ff, TRUE, 0x0300f5},
        {"gray HD", 580, 0x7e, 0x80, 0x80, 0x808080},
        {"past black HD", 580, 0x00, 0x80, 0x80, 0x000000},
        {"past white HD", 580, 0xff, 0x80, 0x80, 0xffffff},
        {"zeros HD", 580, 0x00, 0x00, 0x00, 0x004d00, TRUE, 0x008700},
    };
    ...
    for (i = 0; i < ARRAY_SIZE(test_textures); ++i)
    {
        winetest_push_context("%s format", test_textures[i].name);

        surface_desc.ddpfPixelFormat.dwFourCC = test_textures[i].fourCC;

        for (j = 0; j < ARRAY_SIZE(test_colors); j++)
        {
            winetest_push_context("color %s", test_colors[j].name);

            if (!yuv_surface)
            {
                surface_desc.dwHeight = test_colors[j].height;
            ...
            hr = IDirectDrawSurface7_Lock(yuv_surface, NULL, &lock_desc, DDLOCK_WRITEONLY | DDLOCK_WAIT | DDLOCK_SURFACEMEMORYPTR, NULL);
            ...
            ok(lock_desc.ddpfPixelFormat.dwFourCC == test_textures[i].fourCC,
                    "Got unexpected pixel format %#lx, expected %#lx.\n",
                    lock_desc.ddpfPixelFormat.dwFourCC, test_textures[i].fourCC);

            ptr = (BYTE*)lock_desc.lpSurface + test_textures[i].y_offset;

            for (y = 0; y < lock_desc.dwHeight; y++)
            {
                for (x = 0; x < lock_desc.lPitch / test_textures[i].y_increment; x++)
                {
                    *ptr = test_colors[j].y;
                    ptr += test_textures[i].y_increment;
                }
            }

            chroma_pitch = lock_desc.lPitch / (test_textures[i].h_subsampling * test_textures[i].y_increment);
            chroma_height = lock_desc.dwHeight / test_textures[i].v_subsampling;

            ptr = (BYTE*)lock_desc.lpSurface + test_textures[i].u_offset;

            if (test_textures[i].u_plane)
            {
                ptr += lock_desc.lPitch * lock_desc.dwHeight;
                ptr += chroma_pitch * chroma_height * (test_textures[i].u_plane - 1);
            }

            for (y = 0; y < chroma_height; y++)
            {
                for (x = 0; x < chroma_pitch; x++)
                {
                    *ptr = test_colors[j].u;
                    ptr += test_textures[i].u_increment;
                }
            }

            ptr = (BYTE*)lock_desc.lpSurface + test_textures[i].v_offset;
            if (test_textures[i].v_plane)
            {
                ptr += lock_desc.lPitch * lock_desc.dwHeight;
                ptr += chroma_pitch * chroma_height * (test_textures[i].v_plane - 1);
            }
            ...
            for (y = 0; y < chroma_height; y++)
            {
                ...
                {
                    *ptr = test_colors[j].v;
                    ptr += test_textures[i].v_increment;
                }
            }
            ...
            /* check the top left 4x4 pixels */
            for (y = 0; y < 4; y++)
            {
                pixel = (UINT32*)((BYTE*)lock_desc.lpSurface + lock_desc.lPitch * y);
                for (x = 0; x < 4; x++)
                    ok(compare_color(pixel[x] & 0xffffff, test_colors[j].expected, 1) ||
                            broken(test_colors[j].test_for_broken && compare_color(pixel[x] & 0xffffff, test_colors[j].broken_value, 1)),
                            "Got unexpected value at (%d,%d). Got %#lx, expected %#lx.\n",
                            x, y, (long unsigned)pixel[x] & 0xffffff, (long unsigned)test_colors[j].expected);
            }

            hr = IDirectDrawSurface7_Unlock(rgb_surface, NULL);
            ok(hr == S_OK, "Got unexpected hr %#lx, expected S_OK.\n", hr);

            if (j + 1 == ARRAY_SIZE(test_colors) || test_colors[j].height != test_colors[j+1].height)
            {
