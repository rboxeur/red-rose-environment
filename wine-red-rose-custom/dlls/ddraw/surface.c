...
static ULONG ddraw_surface_release_iface(struct ddraw_surface *This)
{
    ...
    if (iface_count == 0)
    {
        ...
        if ((This->surface_desc.ddsCaps.dwCaps & DDSCAPS_PRIMARYSURFACE) && (This->ddraw->flags & DDRAW_RESTORE_MODE)
                && This->ddraw->swapchain_window)
...
static HRESULT ddraw_surface_attach_surface(struct ddraw_surface *This, struct ddraw_surface *Surf)
{
    ...
    if (surface_attachable_for_flip(Surf))
    {
        ...
        Surf->detach_clear_flags = DDSCAPS_FLIP;
        ...
        if (!This->complex_array[0])
        {
            ...
            else if (This->surface_desc.ddsCaps.dwCaps & DDSCAPS_BACKBUFFER)
            {
                Surf->surface_desc.ddsCaps.dwCaps |= DDSCAPS_FRONTBUFFER;
            }
        }
        ...
        struct ddraw_surface *attachment, IUnknown *detach_iface)
{
    ...
    if (attachment->surface_desc.ddsCaps.dwCaps & DDSCAPS_FLIP)
    {
        ...
        {
            attachment->surface_desc.ddsCaps.dwCaps &= ~DDSCAPS_FRONTBUFFER;
            surface->is_chain_start = 0;
            surface->complex_array[0] = NULL;
            surface->surface_desc.ddsCaps.dwCaps &= ~(DDSCAPS_FLIP | DDSCAPS_FRONTBUFFER);
        }
        else
        {
            if (attachment->surface_desc.ddsCaps.dwCaps & DDSCAPS_FRONTBUFFER)
                surface->complex_array[0]->detach_clear_flags |= DDSCAPS_FRONTBUFFER;
            else
                surface->complex_array[0]->detach_clear_flags |= DDSCAPS_BACKBUFFER;
            surface->complex_array[0]->surface_desc.ddsCaps.dwCaps |= surface->complex_array[0]->detach_clear_flags;
        }
        attachment->surface_desc.ddsCaps.dwCaps &= ~attachment->detach_clear_flags;
