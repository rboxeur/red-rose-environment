...
{
    unsigned char *buffer_end = (unsigned char *)pStubMsg->RpcMsg->Buffer + pStubMsg->BufferLength;
    if (pStubMsg->Buffer > buffer_end || buffer_end - pStubMsg->Buffer < size)
        RpcRaiseException(RPC_X_BAD_STUB_DATA);
...
{
    if (pStubMsg->BufferEnd < pStubMsg->Buffer || pStubMsg->BufferEnd - pStubMsg->Buffer < size)
    {
...
static void validate_string_data(MIDL_STUB_MESSAGE *pStubMsg, ULONG bufsize, ULONG esize)
{
    ...
    if (pStubMsg->BufferEnd < pStubMsg->Buffer || pStubMsg->BufferEnd - pStubMsg->Buffer < bufsize)
    {
