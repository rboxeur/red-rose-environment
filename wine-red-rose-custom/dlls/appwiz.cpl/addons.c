...
static WCHAR *get_cache_file_name(BOOL ensure_exists)
{
    ...
    if (xdg_dir && *xdg_dir && p_wine_get_dos_file_name)
    {
        ...
        xdg_dir_len = WideCharToMultiByte( CP_UNIXCP, 0, xdg_dir, -1, NULL, 0, NULL, NULL );
        xdg_dir_mb = malloc(xdg_dir_len);
        if (xdg_dir_mb)
        {
            WideCharToMultiByte( CP_UNIXCP, 0, xdg_dir, -1, xdg_dir_mb, xdg_dir_len, NULL, NULL );

            cache_dir = p_wine_get_dos_file_name( xdg_dir_mb );
